...
retVal = oRpt.nSetValue("Tag$", "TABLE=SO_SALESORDERHEADER; COLUMN=BATCHNO$")
retVal = oRpt.nSetValue("Operand$", "A")
retVal = oRpt.nWrite()

oRpt.nReportType = 6
retval = oRpt.nInitReportEngine()
retVal = oRpt.nSetExportOptions(5, "\\CUSTOMDEV1\Sage\Sagev2023\Paperless\InvoicePrinting_" & Year(today) & Right("0" & Month(today),2) & Right("0" & Day(today),2) & ".pdf")
